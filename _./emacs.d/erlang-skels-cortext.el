...
(defvar erlang-skel
  '(("If"            "if"            erlang-skel-if)
    ...
    ("Cortext header" "cortext-header" 
     erlang-skel-cortext-header erlang-skel-header)
    ()
    ...
    ("Cortext IQ service" "cortext-ej-service"
     erlang-skel-cortext-ej-service erlang-skel-header)
    ("Ejabberd gen_mod" "ejabberd-gen-mod"
     erlang-skel-cortext-ej-mod erlang-skel-header)
  )
...
;;;; Cortext additions
(defvar erlang-skel-cortext-header
  '(o (erlang-skel-include erlang-skel-copyright-comment)
      ...
      (erlang-skel-include erlang-skel-author)))
(defvar erlang-skel-cortext-ej-includes
  '(o "-include(\"ejabberd.hrl\")." n
      "-include(\"jlib.hrl\")." n
      "-include(\"cortext.hrl\")."))
(defun erlang-cortext-get-ej-service-from-module (module)
  "The module name, with 'cortext_service_' snipped off the front and underscores to dashes"
  (subst-char-in-string ?_ ?- (substring module (length "cortext_service_"))))
(defun erlang-cortext-get-ej-service-from-file-name (&optional file)
  (erlang-cortext-get-ej-service-from-module (erlang-get-module-from-file-name file)))
(defvar erlang-skel-cortext-ej-service-xml
  '(o "#xmlelement{name = \"" (erlang-cortext-get-ej-service-from-file-name) "\"}"))
(defvar erlang-skel-cortext-ej-service
  '(o (erlang-skel-include erlang-skel-cortext-header)
      (erlang-skel-include erlang-skel-cortext-ej-includes) n n
      "-behaviour(cortext_service)." n n
      "-export([service_handler/5])." n n n
      (erlang-skel-separator-start 2)
      "%% CORTEXT_SERVICE CALLBACKS" n
      "%%" n
      "%% Service handlers for 'get' and 'set' requests should " n
      "%%  return a list of XML stanzas on success." n
      "%% They may return '{error, ?ERR_BAD_REQUEST}' to " n
      "%%  indicate an error in the client's request." n
      "-spec service_handler(JID :: #jid{}," n>
		"DID :: device_id()," n> 
		"Type :: atom()," n> 
		"Stanza :: #xmlelement{}," n>
		"Options :: options_list())." n
      "service_handler(_JID, _DID, set, "
            (erlang-skel-include erlang-skel-cortext-ej-service-xml) 
	    " = _Stanza, _Options) ->" n>
      (erlang-skel-include erlang-skel-cortext-ej-service-xml) ";" n n
      "service_handler(_JID, _DID, get, "
           (erlang-skel-include erlang-skel-cortext-ej-service-xml) 
	    " = _Stanza, _Options) ->" n>
      (erlang-skel-include erlang-skel-cortext-ej-service-xml) "." n))
(defvar erlang-skel-cortext-ej-mod
  '(o (erlang-skel-include erlang-skel-cortext-header)
      (erlang-skel-include erlang-skel-cortext-ej-includes) n n
      "-behaviour(gen_mod)." n n
      "-export([start/2, stop/1])." n n n
      (erlang-skel-separator-start 2)
      "%% GEN_MOD CALLBACKS"  n
      "-spec start(Host :: hostname(), Opts :: options_list()) -> any()."
      "start(_Host, _Opts) ->" n>
      "ok." n n
      "-spec stop(Host :: hostname()) -> any()."
      "stop(_Host) ->" n>
      "ok." n n
      (erlang-skel-separator-start 2) n))
